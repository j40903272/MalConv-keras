...
parser.add_argument('--score_path', type=str, default = 'saved/adversarial_score.csv', help="[csv file] Adv sample prediction score")
...
def gen_adv_samples(model, fn_list, pad_percent=0.1, step_size=0.001):
    
    ###   search for nearest neighbor in embedding space ###
    def emb_search(org, adv, pad_idx, pad_len):
        out = org.copy()
        for idx in range(pad_idx, pad_idx+pad_len+1):
            target = adv[idx]
            min_dist = 9999999
            best_idx = 0
    ...
    for e, fn in enumerate(fn_list):
        ...
        pad_idx = len_list[0]
        pad_len = int(len_list[0]*pad_percent)
        if pad_idx >= max_len:
            print ('exceed max len!  Ignored')
            pred_score.append(inp)
            continue
        
        adv, gradient, loss = fgsm(model, inp_emb, pad_idx, pad_len, e, step_size)
        inp_adv = emb_search(inp, adv[0], pad_idx, pad_len)
        pred = model.predict(inp_adv)[0]
        print ('FILE:', fn)
        print ('\tlength %d, pad length %d' % (len_list[0], pad_len))
        print ('\tloss: %f, predict score: %f' % (loss, pred))
        
        pred_score.append(pred)
        # sequence to bytes
        bin_adv = bytes(list(inp_adv[0][:pad_idx+pad_len]))
        adv_samples.append(bin_adv)
        
    return adv_samples, pred_score
...
if __name__ == '__main__':
    ...
    adv_samples, pred_score = gen_adv_samples(model, fn_list, args.pad_percent, args.step_size)
    
    # write to file
    df['adv_score'] = pred_score
    df.to_csv(args.score_path, header=None)
